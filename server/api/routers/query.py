...
@handle_route_errors("Ошибка подсчёта запросов")
...
@handle_route_errors("Ошибка получения данных запроса")
...
@handle_route_errors("Ошибка обработки запроса")
...
async def find_by_email(
    ...
    logger.info("Производится поиск по email")
    Authorize.jwt_required()
    user_id = int(Authorize.get_jwt_subject())
...
async def find_by_company(
    ...
    logger.info("Производится поиск по компании")
    Authorize.jwt_required()
    user_id = int(Authorize.get_jwt_subject())
...
async def find_by_irbis(
    ...
    logger.info("Производится поиск с помощью IRBIS")
    Authorize.jwt_required()
    user_id = int(Authorize.get_jwt_subject())
...
@handle_route_errors("Внутренняя ошибка сервера")
...
@handle_route_errors("Ошибка при скачивании запроса")
...
@handle_route_errors("Не удалось получить языки")
async def get_available_languages(
    ...
    return {russian_name: code for russian_name, code in languages}


@router.post("/query_data", response_model=List[Union[NameQueryDataResult, QueryDataResult]])
@handle_route_errors("Ошибка при получении данных запроса")
async def get_query_data(
    ...
    query = await UserQueriesDAO.get_query_by_id(user_id, request_data.query_id, db)
    if not query:
        raise HTTPException(status_code=404, detail="Запрос не найден или недоступен")
    ...
    for data_item, keyword_type in query_data:
        ...
        if keyword_type_name == 'free word':
            keywords_list = [kw_assoc.keyword for kw_assoc in data_item.keywords]
        else:
            keywords_list = [
        ...
        if query.query_category != 'name':
            ...
            result_item = NameQueryDataResult(
                title=data_item.title,
                info=data_item.info,
                url=data_item.link,
                publication_date=data_item.publication_date,
...
async def get_category_query_data(
    ...
    if total == 0:
        return QueryDataResponse(total=0, size=request_data.size, total_pages=0)

    total_pages = (total + request_data.size - 1) // request_data.size

    if query.query_category != 'name':
        return QueryDataResponse(total=total, size=request_data.size, total_pages=total_pages)
