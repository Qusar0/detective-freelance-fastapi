...
@handle_route_errors("Неожиданная ошибка при получении данных банкротств")
async def get_query_data(
    ...
    return BankruptcyDataResponse(cases=cases, total_count=total_count, total_pages=total_pages)


@router.get("/case_full/{case_id}", response_model=BankruptcyCaseFull)
@handle_route_errors("Неожиданная ошибка при получении дела банкротства")
async def get_full_case_info(
    case_id: int,
    Authorize: AuthJWT = Depends(),
    db: AsyncSession = Depends(get_db),
):
    """Получает полную информацию о банкротстве по ID."""
