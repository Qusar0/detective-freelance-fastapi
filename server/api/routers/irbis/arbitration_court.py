...
@handle_route_errors("Неожиданная ошибка при получении данных арбитражных судов")
async def get_query_data(
    ...
    logger.info(
        ...
        f"search_type: {request_data.search_type}, role: {request_data.role}"
    )
    ...
    cases = [
        ArbitrationCourtCase(
            ...
            inn=case.inn,
            case_type=CaseTypeInfo(id=case.case_type.id, name=case.case_type.name) if case.case_type else None,
            role=RoleTypeInfo(id=case.role.id, name=case.role.russian_name)
        )
        for case in results
    ]
    total_pages = (total_count + request_data.size - 1) // request_data.size if request_data.size > 0 else 0
    return ArbitrationCourtDataResponse(cases=cases, total_count=total_count, total_pages=total_pages)


@router.get("/case_full/{case_id}", response_model=ArbitrationCourtCaseFull)
@handle_route_errors("Неожиданная ошибка при получении дела арбитражного суда")
async def get_full_case_info(
    ...
    return ArbitrationCourtCaseFull(
        id=case.id,
        court_name=case.court_name_val,
        case_date=case.case_date,
        name=case.name,
