...
from server.api.schemas.irbis.statistic import StatisticGeneralCase
...
from server.api.routers.irbis.tax_arrears import router as tax_arrears_router
...
router.include_router(tax_arrears_router)


@router.get("/regions", response_model=List[RegionInfo], tags=['Irbis/Общее'])
@handle_route_errors("Неожиданная ошибка при получении регионов")
async def get_regions(db: AsyncSession = Depends(get_db)):
    """Получает информацию о всех регионах РФ."""
    regions = await RegionSubjectDAO.find_all(db)
    return [RegionInfo(id=region.subject_number, name=region.name) for region in regions]


@router.get("/person_info/{query_id}", response_model=IrbisPersonInfo, tags=['Irbis/Общее'])
@handle_route_errors("Неожиданная ошибка при получении информации о человеке")
async def get_person_info(
    ...
    person_info = IrbisPersonInfo(
        ...
        regions=[RegionInfo(id=region.id, name=region.name) for region in regions]
    )
...
@handle_route_errors("Неожиданная ошибка при получении статистики")
async def get_query_data(
    ...
    logger.info(f"Запрос statistic для query_id: {query_id}")
    Authorize.jwt_required()
    user_id = int(Authorize.get_jwt_subject())
    logger.debug(f"Аутентифицированный пользователь: {user_id}")
    ...
    results = await StatisticsDAO.get_all_counts(person_id=irbis_person.id, db=db)
    return StatisticGeneralCase(
